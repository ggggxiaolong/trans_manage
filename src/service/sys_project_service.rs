...
impl SysProjectService {
    pub async fn all_project(&self) -> Result<Vec<VOProject>, CustomError> {
        let projects: Vec<Project> = sql::project::query_all().await?;
        Ok(projects.into_iter().map(|p| p.into()).collect())
    }

    pub async fn merge_update(&self, project_id: i32, user_id: i32) -> Result<i32, CustomError> {
        // transaction.co
